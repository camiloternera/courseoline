<?php include_once "includes/template/header.php"; ?>

  <section class="hero">
    <div class="container-hero container">
      <h2 class="text-hero">Bienvenido a <br> Funcatec</h2>
      <p class="text">
        <span class="typed"></span>
      </p>
      <div class="text" id="text">
        <p>Capacitate con nosotros</p>
        <p>Da el primer paso para cumplir tus sueños</p>
      </div>
    </div>
  </section>

  <section class="seccion">
    <div class="content-welcome container">
      <div class="imagen-course">
        <img src="img/course.jpeg" alt="">
      </div>
      <div class="parrafo-welcome">
        <h2 class="texto">Bienvenido a Funcate</h2>
        <p class="parrafo">La fundacion social para la capacitacion y desarrollo tecnologico de belleza y artes en colombia, es una fundacion sin animo de lucro que ofrece no solo la oportunidad de aprender un arte u oficio, sino la orientacion necesaria para que los estudiantes se motiven a ejercer o llevar a la practica los conocimientos y tecnicas enseñadas.</p>
        <a href="about.php" class="button">Leer más</a>
      </div>
    </div><!-- .content-welcome -->
  </section><!-- .seccion -->

  <section class="container seccion">
    <div class="about-index">
      <div class="about-index__content">
        <img src="img/p1.jpg" alt="">
        <p class="about-index__text c-texto">Aprende cosas nuevas</p>
      </div>
      <div class="about-index__content">
        <img src="img/p2.jpg" alt="">
        <p class="about-index__text c-texto">Cursos certificados</p>
      </div>
      <div class="about-index__content">
        <img src="img/p3.jpg" alt="">
        <p class="about-index__text c-texto">Trabaja tu creatividad</p>
      </div>
    </div>
  </section>

  <section class="images-content">
    <div class="container content-contact">
      <h2 class="texto">Encuentra los mejores cursos</h2>
      <p>Lorem ipsum dolor, sit, amet consectetur adipisicing elit.</p>
      <a href="contacto.php" class="button">Inscripciones</a>
    </div>
  </section>

  <main class="courses--next seccion container">
    <h2 class="texto c-texto m-4rem">Algunos cursos</h2>
    <div class="courses__container--next">
      <div class="course">
        <img src="img/p7.jpg" alt="Curso de ingles">
        <div class="course__content">
          <h3>Nombre del cursos</h3>
          <p>Lorem ipsum dolor, sit amet consectetur adipisicing, elit. Iusto debitis rerum tempore asperiores
            repellendus laboriosam vitae ipsam, velit ea impedit neque dolores labore odio deleniti repellat eaque porro
            iure corrupti.</p>
          <a href="cursos.php" class="button">Ir al curso</a>
        </div>
      </div>
      <div class="course">
        <img src="img/p8.jpg" alt="Curso de Excel">
        <div class="course__content">
          <h3>Nombre del cursos</h3>
          <p>Lorem ipsum dolor, sit amet consectetur adipisicing, elit. Iusto debitis rerum tempore asperiores
            repellendus laboriosam vitae ipsam, velit ea impedit neque dolores labore odio deleniti repellat eaque porro
            iure corrupti.</p>
          <a href="cursos.php" class="button">Ir al curso</a>
        </div>
      </div>
      <div class="course">
        <img src="img/p9.jpg" alt="Curso de PhotoShop">
        <div class="course__content">
          <h3>Nombre del cursos</h3>
          <p>Lorem ipsum dolor, sit amet consectetur adipisicing, elit. Iusto debitis rerum tempore asperiores
            repellendus laboriosam vitae ipsam, velit ea impedit neque dolores labore odio deleniti repellat eaque porro
            iure corrupti.</p>
          <a href="cursos.php" class="button">Ir al curso</a>
        </div>
      </div>
      <div class="course">
        <img src="img/p10.jpg" alt="Curso de PhotoShop">
        <div class="course__content">
          <h3>Nombre del cursos</h3>
          <p>Lorem ipsum dolor, sit amet consectetur adipisicing, elit. Iusto debitis rerum tempore asperiores
            repellendus laboriosam vitae ipsam, velit ea impedit neque dolores labore odio deleniti repellat eaque porro
            iure corrupti.</p>
          <a href="cursos.php" class="button">Ir al curso</a>
        </div>
      </div>
    </div>
  </main>

  <section class="seccion mapa">
    <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3917.0382121603448!2d-74.78340268566197!3d10.96048615876748!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8ef42d55fd37847f%3A0x20c86e8cd161f9d8!2sCra.%2021d%20%2325-71%2C%20Barranquilla%2C%20Atl%C3%A1ntico!5e0!3m2!1ses!2sco!4v1613322073938!5m2!1ses!2sco" width="100%" height="450" frameborder="0" style="border:0;" allowfullscreen="" aria-hidden="false" tabindex="0"></iframe>
  </section>

<?php include_once "includes/template/footer.php"; ?>
